module Codewars.G964.SharesPriceSpec where

import Codewars.G964.SharesPrice (sharePrice)
import Test.Hspec
...
spec = do
  describe "sharePrice" $ do
    it "returns the original price if there was no change" $
      property $ \x ->
        sharePrice x [] `shouldBe` printf "%.2f" x

    it "returns the original price if the change is 0%" $
      property $ \x ->
        forAll (listOf $ pure 0) $ \xs ->
          sharePrice x xs `shouldBe` printf "%.2f" x

    it "returns half the price if the change is -50" $
      property $ \x ->
        sharePrice x [-50] `shouldBe` printf "%.2f" (x / 2)

    it "returns double the price if the change is 100" $
      property $ \x ->
        sharePrice x [100] `shouldBe` printf "%.2f" (2 * x)

    it "returns 0 if the change is -100 at any point" $
      property $ \x ->
