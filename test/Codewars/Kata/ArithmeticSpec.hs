module Codewars.Kata.ArithmeticSpec where
import Codewars.Kata.Arithmetic (findMissing)
import Test.Hspec
...
spec = do
  describe "findMissing" $ do
    ...
    it "should work for some simple negative numbers" $ do
      findMissing [0, -2, -3] `shouldBe` -1
